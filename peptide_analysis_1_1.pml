...
# version 1.1 - beta (with commands for direct labeling of residues onto peptide structure)
...
#   A. put Plugin file (peptide_analysis_1_1.pml) in a destination folder, from which you want to run it (e.g. C:\User\Max_Muster\Desktop\PyMOL_Plguins) --> copy and store the pathway information for later running the plugin  
...
#   F. type "run <PATH>/peptide_analysis_1_1.pml" in command line at the top
...
# now we label all amino acids of the peptide one by one adjacent to their C-alpha atoms with their 3-letter name and position 
    select selection_2, pep_of_int;
    label selection_2 and n. ca, "%s%s"%(resn,resi);
    set label_placement_offset, [0.0,1.0,1.0];
    set label_font_id, 15;
    set label_size, -0.8;
    set label_bg_color, white; 
    set label_bg_transparency, 0.2;
    set label_connector_color, green;
    set label_bg_outline, on;
...
# to rule out interferences of the labels with the electrostatic surfaces generated in the next step, we temporary hide the labels 
    hide label;
...
# at last we show the labels again, so you can check them in the PyMOL viewer
    label selection_2 and n. ca, "%s%s"%(resn,resi);
    delete selection_2;
    set label_placement_offset, [0.0,1.0,1.0];
    set label_font_id, 15;
